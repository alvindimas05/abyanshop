...
class PembelianController extends Controller
{
    ...
    public function isUserExist(string $id){
        return DB::table("users")->where("user_id", "=", $id)->exists();
    }
    // user_id, id_produk 
    public function beli(Request $req){
        ...
        $user = DB::table("users")->where("user_id", "=", $req->user_id)->get(["saldo"])->first();
        $produk = DB::table("produk")->where("id", "=", $req->id_produk)->get(["harga", "total_penjualan"])->first();
        
        if($user->saldo < $produk->harga) return $this->res->failed("Saldo tidak cukup!");
        DB::table("users")->where("user_id", "=", $req->user_id)->update([ "saldo" => $user->saldo - $produk->harga ]);
        DB::table("produk")->where("id", "=", $req->id_produk)->update([ "total_penjualan" => $produk->total_penjualan ]);
    ...
    public function top_up(Request $req){
        ...
        DB::table("users")->where("user_id", "=", $req->user_id)->update([ "saldo" => DB::raw("saldo + ".$req->jumlah) ]);
        return $this->res->success();
    }
}
